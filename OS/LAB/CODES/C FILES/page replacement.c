...
return hit; }
int getHitIndex(int data) { int hitind;
for (k = 0; k < nf; k++)
{
if (p[k] == data) {
hitind = k; break; }
...
void dispPgFaultCnt() {
printf("\nTotal no of page faults:%d", pgfaultcnt); } void fifo() {
initialize();
for (i = 0; i < n; i++) {
...
p[k] = in[i]; printf("\tf"); pgfaultcnt++; dispPages(); } else
printf("\t"); }
dispPgFaultCnt(); }
void lru() {
initialize();
...
for (i = 0; i < n; i++) {
 ...
 printf("\nFrame %d :", in[i]); if (isHit(in[i]) == 0) {
for (j = 0; j < nf; j++) {
int pg = p[j];
int found = 0;
for (k = i - 1; k >= 0; k--) {
if (pg == in[k]) {
least[j] = k; found = 1; break; } else
found = 0; }
